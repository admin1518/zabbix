...
software=/tmp/software.log
dateline=`date +%F`
# 清空日志
echo "dateline" > ${software}
# 同步服务器时间
date_sys() {
...
if [ $release = 7 ];then
	...
	yum remove php.x86_64 php-cli.x86_64 php-common.x86_64 php-gd.x86_64 php-ldap.x86_64 php-mbstring.x86_64 php-mcrypt.x86_64 php-mysql.x86_64 php-pdo.x86_64 -y >> ${software} 2>&1
	rpm -Uvh http://mirrors.isu.net.sa/pub/fedora/fedora-epel/6/i386/epel-release-6-8.noarch.rpm >> ${software} 2>&1
	rpm -Uvh http://mirror.webtatic.com/yum/el6/latest.rpm >> ${software} 2>&1
	yum -y install unixODBC patch unixODBC-devel patch java-devel wget unzip libxml2 libxml2-devel httpd mysql mysql-server  curl curl-devel net-snmp net-snmp-devel perl-DBI ntpdate zlib-devel mysql-devel glibc-devel gcc-c++ gcc automake mysql libidn-devel openssl-devel net-snmp-devel rpm-devel OpenIPMI-devel php56w.x86_64 php56w-cli.x86_64 php56w-common.x86_64 php56w-gd.x86_64 php56w-ldap.x86_64 php56w-mbstring.x86_64 php56w-mcrypt.x86_64 php56w-mysql.x86_64 php56w-pdo.x86_64 php56w-bcmath php56w-xml -y >> ${software} 2>&1
	[ $? -eq 0 ] && echo -e "\033[32m software installed \033[0m" || echo -e "\033[31m [Error]:software installed failed, Pls check \033[0m"
...
echo "create database IF NOT EXISTS zabbix default charset utf8 COLLATE utf8_general_ci;" | mysql -uroot -p123
echo "grant all privileges on zabbix.* to zabbix@'localhost' identified by 'zabbix';" | mysql -uroot -p123
echo "flush privileges;" | mysql -uroot -p123
...
cp -ra $zabbixdir/zabbix-${zabbix_version}/frontends/php/* /var/www/html/zabbix
...
echo "Mysql默认root密码为123 ; zabbix-Database name:zabbix/User:zabbix/Password:zabbix"
